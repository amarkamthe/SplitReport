...
def generate():
	...
	for (group, values) in stock.items():
		...
		dfs['% change'] = dfs['% of current'] - dfs['% of invested']
		data.append([group, total_invested_price_dfs, total_current_price_dfs])
		df = dfs[['Symbol', 'Quantity Available', 'Average Price', 'Invested Amt', 'Current Amt', 'Abs return', '% of invested', '% of current', '% change' ]]
		write_to_excel(df, group)

	df_tmp = pd.DataFrame(data, columns = ['Name', 'Invested Amt', 'Current Amt'])
	df_tmp['Abs return'] = round(((df_tmp['Current Amt'] - df_tmp['Invested Amt']) / df_tmp['Invested Amt']) * 100, 2)
	...
	df_tmp['% change'] = df_tmp['% of current Amt'] - df_tmp['% of invested Amt']
